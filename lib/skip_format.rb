class SkipFormat < RequestLogAnalyzer::FileFormat::Rails
  ...
  report(:append) do |analyze|
    analyze.frequency :category => REQUEST_CATEGORIZER_PER_CURRENT_USER, :title => "Search requested per user", :line_type => :current_user, :if => lambda { |request| request[:controller] == 'SearchController' && request[:action] == 'full_text_search' }
    analyze.frequency :category => REQUEST_CATEGORIZER_PER_CURRENT_TARGET_USER, :title => "User profile requested", :line_type => :current_user, :if => lambda { |request| request[:controller] == 'UserController' && request[:action] == 'show' }
  end
end
